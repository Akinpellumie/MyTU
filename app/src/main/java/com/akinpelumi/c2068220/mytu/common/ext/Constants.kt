package com.akinpelumi.c2068220.mytu.common.ext

object Constants {
    //App
    const val TAG = "AppTag"

    //Buttons
    const val SIGN_IN_BUTTON = "Sign in"
    const val EDIT_INFO = "Edit Information"
    const val SAVE = "Save Changes"
    const val RESET_PASSWORD_BUTTON = "Reset"
    const val SIGN_UP_BUTTON = "Sign up"

    //Menu Items
    const val SIGN_OUT_ITEM = "Sign out"
    const val REVOKE_ACCESS_ITEM = "Revoke Access"

    //Screens
    const val SIGN_IN_SCREEN = "Sign in"
    const val FORGOT_PASSWORD_SCREEN = "Forgot password"
    const val EDIT_INFO_SCREEN = "Edit Information"
    const val SIGN_UP_SCREEN = "Sign up"
    const val VERIFY_EMAIL_SCREEN = "Verify email"
    const val PROFILE_SCREEN = "Profile"
    const val ATTENDANCE_SCREEN = "My Attendance"
    const val TIMETABLE_SCREEN = "Timetable Agenda"
    const val TODO_SCREEN = "To-Do List"
    const val BALANCE_SCREEN = "Balances"
    const val LIBRARY_SCREEN = "Library"
    const val MODULE_SCREEN = "Modules"

    //Labels
    const val EMAIL_LABEL = "Email"
    const val PASSWORD_LABEL = "Password"

    //Placeholders
    const val NAME_LABEL = "Enter Display name"

    //Useful
    const val EMPTY_STRING = ""
    const val VERTICAL_DIVIDER = "|"

    //Texts
    const val FORGOT_PASSWORD = "Forgot password? Click to get recovery email"
    const val NO_ACCOUNT = "New here? Create Account"
    const val ALREADY_USER = "Already a user? Sign in."
    const val CLASS_CODE = "Verification code"
    const val WELCOME_MESSAGE = "Welcome to our app."
    const val TAKE_PHOTO = "Take a Photo"
    const val ALREADY_VERIFIED = "Already verified?"
    const val INVALID_EMAIL = "Please Insert a valid email."
    const val NO_PASSWORD_MATCH = "Passwords do not match."
    const val INVALID_PASSWORD = "PYour password should have at least six digits and include one digit, one lower case letter and one upper case letter."
    const val SPAM_EMAIL = "If not, please also check the spam folder."

    //Messages
    const val VERIFY_EMAIL_MESSAGE = "We've sent you an email with a link to verify the email."
    const val EMAIL_NOT_VERIFIED_MESSAGE = "Your email is not verified."
    const val RESET_PASSWORD_MESSAGE = "We've sent you an email with a link to reset the password."
    const val REVOKE_ACCESS_MESSAGE = "You need to re-authenticate before revoking the access."
    const val ACCESS_REVOKED_MESSAGE = "Your access has been revoked."
    const val EDIT_INFO_MESSAGE = "Your profile has been updated successfully."
    const val CLASS_CODE_ERROR_MESSAGE = "Invalid class code or ENTRY empty."
    const val CLASS_CODE_MESSAGE = "Attendance Registered Successfully."

    //Error Messages
    const val SENSITIVE_OPERATION_MESSAGE = "This operation is sensitive and requires recent authentication. Log in again before retrying this request."

    //google signin-credentials
    const val GOOGLE_WEB_CLIENT_ID = "1006781693646-t8g0fb6nqcir1as25e61488tuhva98ju.apps.googleusercontent.com"
}